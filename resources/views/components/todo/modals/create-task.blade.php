...
<div x-show="{{ $show }}" x-transition.opacity class="fixed inset-0 z-40 flex items-center justify-center bg-slate-900/60 p-4" x-cloak @click.self="{{ $show }} = false">
    <div x-show="{{ $show }}" x-transition.scale class="relative w-full max-w-2xl rounded-2xl bg-white p-6 shadow-xl dark:bg-slate-900">
        ...
        <form method="POST" action="{{ route('todos.store') }}" class="mt-6 space-y-4">
            ...
            <div class="flex items-center justify-end gap-3">
                <button type="button" @click="{{ $show }} = false" class="rounded-full border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-800">キャンセル</button>
                <button type="submit" class="rounded-full bg-rose-500 px-6 py-2 text-sm font-semibold text-white hover:bg-rose-600">追加する</button>
            </div>
        </form>
    </div>
</div>
